...
int** CrfViterbi(ProcessedInput* processedInput, double ** rfProbs, int** windowCalls, double** marginals, int emIteration){
  ...
  for(int windex=0; windex < numWindows-1; windex++){
    ...
    double thisWindowHalf = ((*windowEndLocs)[windex] - (*windowBeginLocs)[windex])/2.0;
  ...
  for(int windex=0; windex < numWindows-1; windex++){
    transitionProbs[windex] = 1-exp(-distBtWindowsMids[windex]*(genSinceAdmixtureDouble)/200.0);  // divide by 100 bc converting from cM to M
  }
